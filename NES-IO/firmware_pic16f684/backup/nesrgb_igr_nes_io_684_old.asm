...
;                 (green) LED1 out |5  C5 C0 10| rgb-mode: garish   in/out [NESRGB Pad 1]
;                   (red) LED2 out |6  C4 C1  9| rgb-mode: improved in/out [NESRGB Pad 2]
;                      LED_TYPE in |7  C3 C2  8| rgb-mode: natural  in/out [NESRGB Pad 3]
;                                  `-----------'
;
...
;
;   preparation of the CIC
;   ======================
;
;   You have to cut the reset line on the mainboard straight at the reset button. The
;   reset button end goes to pin 3 of the IGR and the other side to pin 2. If pin 7 of
;   the CIC is set to GND during NES' operation you may connect pin 3 of the IGR over
;   a 10kOhm resistor to GND, otherwise to Vcc.
;
;   Once you do something at the pins, you may also deactivate the lockout by lifting
;   pin 4 of the CIC and connecting this pin to GND.
;
...
delay_10ms_t0_overflows EQU 0x28    ; prescaler T0 set to 1:2 @ 8MHz
repetitions_040ms       EQU 0x04
repetitions_200ms       EQU 0x14
repetitions_300ms       EQU 0x1e
repetitions_480ms       EQU 0x30
repetitions_1500ms      EQU 0x96
...
;    M_celf  0xff, reg_ctrl_data, set_reg_current_mode_pre   ; no butto pressed (only needed for change on reset release)
	goto	idle
...
ctrl_reset
    ...
    goto            dolongreset
    M_delay_x10ms   repetitions_040ms
...
doreset
    ...
    M_delay_x10ms   repetitions_480ms
    skipnext_for_lowreset
...
dolongreset
    ...
    M_movff         reg_reset_type, PORTA
    call            set_reg_current_mode
    M_delay_x10ms   repetitions_1500ms
    M_delay_x10ms   repetitions_1500ms
...
;    call    setleds                     ; change mode once ctrl buttons are released
...
domode_next
    ...
    call    set_reg_current_mode_pre                        ; change mode during runtime
;    call    setleds                                         ; change mode once ctrl buttons are released
    M_delay_x10ms   repetitions_mode_delay
    goto            idle
...
check_reset_continue
    M_movlf repetitions_mode_delay , reg_repetition_cnt  ; repeat delay_10ms x-times
    skipnext_for_lowreset
...
check_reset_low
    ...
    decfsz  reg_repetition_cnt, 1   ; delay_10ms repeated x-times?
...
;    goto    reset_mode_change_end       ; no -> end procedure (after reset released change)
    decfsz  reg_repetition_cnt, 1       ; delay_10ms repeated x-times?
...
;reset_mode_change_end   ; only needed for change on reset release
;    call    set_reg_current_mode_pre
;    goto    idle
...
modeset_natural
    clrf    PORTC
    banksel	TRISA       ; Bank 1
    M_movlf 0x0b, TRISC ; out out in out in in
    banksel PORTA       ; Bank 0
    goto    setleds_red
...
modeset_improved
    clrf    PORTC
    banksel TRISA       ; Bank 1
    M_movlf 0x0d, TRISC ; out out in in out in
    banksel PORTA       ; Bank 0
    goto    setleds_green
...
modeset_garish
    clrf    PORTC
    banksel TRISA       ; Bank 1
    M_movlf 0x0e, TRISC ; out out in in in out
    banksel PORTA       ; Bank 0
    goto    setleds_yellow
...
start
    ...
    M_movlf 0x18, INTCON            ; enable interrupts: T0IE, INTE (interrupt on rising/falling edge on A2) and RAIE
    banksel TRISA                   ; Bank 1
